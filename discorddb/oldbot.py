...
async def help(ctx): #defining the function
	...
	embed.add_field(name = '..remove server <number>', value = 'DISPLAYS DETAILS OF SENT TRANSACTIONS', inline = False)
	embed.add_field(name = '..remove <name_of_server> <number>', value = 'DISPLAYS DETAILS OF SENT TRANSACTIONS', inline = False)

	await ctx.channel.send(embed=embed) #SENDING THE MESSAGE ON DISCORD

@client.command(aliases=[' add'])
@commands.has_permissions(administrator=True)
async def add(ctx, name,  field, date):
...
async def listme(ctx):
	...
	"""
		...
		await ctx.channel.send('You dont have data')
